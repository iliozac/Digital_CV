...
1. ***Sincronizzazione con strumenti esterni (synth, master keyboard, sequencer, software DAW, ecc...)***.
   ...
   La sincronizzazione con strumenti esterni può quindi avvenire in due modi: 1) Tramite interfaccia MIDI (o USB midi) e opportuna conversione MIDI->dCV. 2) Direttamente tramite dCV con device esterni che implementano un uscita dCV. 
...
3. ***Gestione di strutture polifoniche***
  ...
  Questo è dovuto a quello che possiamo definire il "peccato originale" della categoria dei modulari, ovvero un sistema di controllo basato su tensioni analogiche: i fatidici segnali "CV". Per un amante dell'elettronica analogica non si può immaginare nulla di più affascinante di una semplice tensione variabile che viene riconosciuta e interpretata in modo pressochè univoco da un intero universo di circuiti (peraltro molti dei quali ormai basati su tecnologia digitale). Se questo è comprensibile tenendo conto 
